...
the Vue playground, and the Nuxt playground together. The launcher gives both
apps the URL generated by that backend, so stale app-level `.env` files cannot
split the playgrounds across deployments. Each surface also has a root command:
`pnpm dev:backend`, `pnpm dev:vue`, and `pnpm dev:nuxt`.
