...
describe('workspace development scripts', () => {
  ...
  it('runs every deterministic gate through verify', async () => {
    ...
    expect(root.scripts['test:live']).toContain('test:live:browser')
    expect(root.scripts['test:live']).toContain('test:live:static')
    expect(root.scripts['test:live:browser']).toContain('playwright.live.config.ts')
    expect(root.scripts['test:live:static']).toContain('verify-nuxt-static.mjs')
